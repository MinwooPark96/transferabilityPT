...
def load_pretrained(model_name):
    ...
    model = AutoModelWithLMHead.from_pretrained(model_name, config=config) # BertForMaskedLM
    model.eval()
...
def main(args):
    ...
    # print(tokenizer.convert_ids_to_tokens(trigger_ids.squeeze(0).tolist())) 
    # MASK 토큰으로 init -> 안바뀌고 계속 mask 로 박힘.
...
if __name__ == '__main__':
    ...
    #python label_search.py --train ./train.tsv --template '[CLS] {sentence} [T] [T] [T] [P]. [SEP]' --label-map '{"0": 0, "1": 1}' --iters 50 --model-name 'bert-base-cased'
    
    #python autoprompt/label_search.py --train ./autoprompt/train.tsv --template '[CLS] {sentence} [T] [T] [T] [P]. [SEP]' --label-map '{"0": 0, "1": 1}' --iters 50 --model-name 'bert-base-cased'
